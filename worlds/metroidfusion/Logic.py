...
def create_logic_rule_for_list(
    ...
    r_e_m_p: tuple[list[set[str]], list[int], list[int], list[int], list[bool]] = ([], [], [], [], [])
    for requirement in requirements:
        for (new_rule,
            ...
            r_e_m_p[4].append(yaml_enabled)
    if debug:
        for (requirement,
             energy_tanks,
             missiles,
             power_bombs,
             yaml_enabled) in zip(*r_e_m_p):
        ...
        if requirement.requirements:
            ...
            for requirements_permutation in requirements_product:
                ...
                if not all([nested_requirement.check_option_enabled(options)
                        for nested_requirement in requirements_permutation]):
                    # If one is disabled, add a possibility with a flag telling that it's false.
                    possibilities.append(( {"False Requirement Flag"}, 0, 0, 0, False ))
                    if debug:
